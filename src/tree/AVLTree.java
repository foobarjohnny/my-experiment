...
/**
 * http://baishukui.blog.163.com/blog/static/9401858920124302553867/
 * 
 * @data 2012-3
 */
public class AVLTree<E extends Comparable<E>> extends AbstractSet<E>
{
	...
	private class TreeIterator implements Iterator<E>
	{
		...
		public E next()
		{
			...
			lastReturned = enext;
			enext = successor(enext);

			// System.out.println(list.getObj()+" list.printLinkList()  "+list.printLinkList());
			// System.out.println(" lastReturned.elem  "+lastReturned.elem);
			return (E) (lastReturned.elem + "  " + lastReturned.totalNum + " ");
	...
	public boolean add(E elem)
	{
		...
		if (root == null) {
			...
			// root.next.add(charset);
			// System.out.println("charset is "+charset+"  test "+root.pnext.printLinkList());
			size++;
			return true;
		}
		else {
			...
			while (true) {// 死循环，直接找到插入点退出循环
				...
				if (comp == 0) {
					tmp.totalNum++;
					// if(!tmp.pnext.find(charset)){
					// tmp.pnext.addList(charset);
					// if(tmp.elem.equals("是") ){
					// System.out.println("xxxxxxcharset is "+tmp.pnext.printLinkList());
					// }
					// }
					return false;
				}
				...
				if (comp < 0) {
					...
					if (tmp.left != null) {
						...
						tmp.left.totalNum++;
						// tmp.left.pnext=new LinkNode(elem);
						// tmp.left.pnext.addList(charset);
						// System.out.println("left charset is "+charset+"  test "+tmp.pnext.printLinkList());
						// 插入后要进行平衡调整
					...
					if (tmp.right != null) {
						...
						tmp.right.totalNum++;
						// tmp.right.pnext=new LinkNode(elem);
						// tmp.right.pnext.addList(charset);
						// System.out.println("right "+"tmp.right.elem is "+tmp.right.elem
						// +"charset is "+charset+"  test "+tmp.pnext.printLinkList());
						// 插入后要进行平衡调整
	...
	protected void fixAfterInsertion(Entry<E> ancestor, Entry<E> inserted)
	{
		...
		if (ancestor == null) {
			 ...
			 * ，树的高度不会发生变化，所以不需要旋转，我们要作的就是调整从根节点到插入节点的路径上的所有 节点的平衡因子罢了
			 * 
			 * 新增55后调整 50 → 50 /=\ /R\ 25 70 25 70 /=\ /=\ /=\ /L\ 15 30 60 90
			 * 15 30 60 90 = = = = = = /L = 55 =
			 */
		...
		else if ((ancestor.balanceFactor == 'L' && elem.compareTo(ancestor.elem) > 0) || (ancestor.balanceFactor == 'R' && elem.compareTo(ancestor.elem) < 0)) {
			/*
			 ...
			 * 新增55后调整 ancestor → 50 → 50 /L\ /=\ 25 70 25 70 /R\ /=\ /R\ /L\ 15
			 * 30 60 90 15 30 60 90 /L /L /L 28 28 55 新增28后调整 ancestor → 50 → 50
			 * /R\ /=\ 25 70 25 70 /=\ /L\ /R\ /L\ 15 30 60 90 15 30 60 90 /L /L
			 * /L 55 28 55
			 */
		...
		else if (ancestor.balanceFactor == 'R' && elem.compareTo(ancestor.right.elem) > 0) {
			/*
			 * 情况3： ancestor.balanceFactor的值为 R，并且被插入的Entry位于ancestor的右子树的右子树上，
			 * 此 种情况下会引起树的不平衡，所以先需绕ancestor进行旋转，再进行平衡因子的调整
			 * 
			 * 新增93 先调整因子再绕70左旋 → 50 → 50 /R\ /R\ 25 70 ← ancestor 25 90 /L /R\
			 * /L /=\ 15 60 90 15 70 98 = = /=\ = /=\ /L 80 98 60 80 93 = /= = =
			 * = 93 =
			 */
			ancestor.balanceFactor = '=';
			// 围绕ancestor执行一次左旋
		...
		else if (ancestor.balanceFactor == 'L' && elem.compareTo(ancestor.left.elem) < 0) {
			/*
			 * 情况4： ancestor.balanceFactor的值为 L，并且被插入的Entry位于ancestor的左子树的左子树上，
			 * 此种情况下会引起树的不平衡，所以先需绕ancestor进行旋转，再进行平衡因子的调整
			 * 
			 * 新增13 先调整因子再绕50右旋 → 50 ← ancestor → 20 /L\ /=\ 20 70 10 50 /=\ /=\
			 * /R\ /=\ 10 30 60 90 5 15 30 70 /=\ /=\= = = /L /=\ /=\ 5 15 25 35
			 * 13 25 35 60 90 = /= = = = = = = = 13 =
			 */
			ancestor.balanceFactor = '=';
			// 围绕ancestor执行一次右旋
	...
	protected void adjustPath(Entry<E> to, Entry<E> inserted)
	{
		...
		while (tmp != to) {
			 ...
			 * 先节点50的平衡因子也被打破，因为在50的左子树上插入的，所以对50来说，左子树会比右子树高1，所 以其平衡因子为L 50 50
			 * /=\ 插入30 /L\ 25 70 → 25 70 = = R\ = 30 =
			 */
	...
	protected void adjustLeftRigth(Entry<E> ancestor, Entry<E> inserted)
	{
		...
		if (ancestor.parent == inserted) {
			 ...
			 * 
			 * 新增40 绕30左旋 绕50右旋 → 50 ← ancestor → 50 → /L /L 30 40 =\ /= 40 30 =
			 * =
			 * 
			 * 调整平衡因子 40 → 40 /=\ /=\ 30 50 30 50 = L = =
			 * 
			 ...
			 * 
			 * 新增35 绕20左旋 → 50 ← ancestor → 50 /L\ /L\ 20 90 40 90 /=\ /=\ /=\
			 * /=\ 10 40 70 100 20 45 70 100 /=\ /=\= = /=\ 5 15 30 45 10 30 = =
			 * =\ = /=\ =\ 35 5 15 35 = = = =
			 * 
			 * 绕50右旋 调整平衡因子 → 40 → 40 /=\ /=\ 20 50 20 50 /=\ /L\ /=\ /R\ 10 30
			 * 45 90 10 30 45 90 /=\ =\ /=\ /=\ R\ /=\ 5 15 35 70 100 5 15 35 70
			 * 100 = = = = = = = = = =
			 */
			 ...
			 * 
			 * 新增42 绕20左旋 → 50 ← ancestor → 50 /L\ /L\ 20 90 40 90 /=\ /=\ /=\
			 * /=\ 10 40 70 100 20 45 70 100 /=\ /=\= = /=\ /= 5 15 30 45 10 30
			 * 42 = = = /= /=\= = 42 5 15 = = =
			 * 
			 * 绕50右旋 调整平衡因子 → 40 → 40 /=\ /=\ 20 50 20 50 /=\ /L\ /L\ /=\ 10 30
			 * 45 90 10 30 45 90 /=\ /= /=\ /=\ /L /=\ 5 15 42 70 100 5 15 42 70
			 * 100 = = = = = = = = = =
			 */
	...
	protected void adjustRigthLeft(Entry<E> ancestor, Entry<E> inserted)
	{
		...
		if (ancestor.parent == inserted) {
			 ...
			 * 第1种，新增的节点在旋转完成后为ancestor父节点情况：
			 * 
			 * 新增40 绕50右旋 绕30左旋 → 30 ← ancestor → 30 → R\ R\ 50 40 /= =\ 40 50 =
			 * =
			 * 
			 * 40 调整平衡因子 40 /=\ → /=\ 30 50 30 50 R = = =
			 * 
			 ...
			 * 
			 * 新增73 绕90右旋 → 50 ← ancestor → 50 /R\ /R\ 20 90 20 70 /=\ /=\ /=\
			 * /=\ 10 40 70 95 10 40 65 90 = = /=\ /=\ = = = /=\ 65 75 93 97 75
			 * 95 = /= = = /= /=\ 73 73 93 97 =
			 * 
			 * 绕50左旋 调整平衡因子 → 70 → 70 /=\ /=\ 50 90 50 90 /R\ /=\ /L\ /=\ 20 65
			 * 75 95 20 65 75 95 /=\ = /= /=\ /=\ = /L /=\ 10 40 73 93 97 10 40
			 * 73 93 97 = = = = = = = = = =
			 */
			 ...
			 * 
			 * 新增63 绕90右旋 → 50 ← ancestor → 50 /R\ /R\ 20 90 20 70 /=\ /=\ /=\
			 * /=\ 10 40 70 95 10 40 65 90 = = /=\ /=\ = = /= /=\ 65 75 93 97 63
			 * 75 95 /= = = = = = /=\ 63 93 97 = = =
			 * 
			 * 绕50左旋 调整平衡因子 → 70 → 70 /=\ /=\ 50 90 50 90 /R\ /=\ /=\ /R\ 20 65
			 * 75 95 20 65 75 95 /=\ /= = /=\ /=\ /L /=\ 10 40 63 93 97 10 40 63
			 * 93 97 = = = = = = = = = =
			 */
	...
	protected void fixAfterDeletion(E elem, Entry<E> ancestor)
	{
		...
		while (ancestor != null && heightHasDecreased) {
			 ...
			 * 
			 * 82 /L\ 42 95 /=\ R\ 33 48 96 /=\ /=\ 29 36 43 75
			 */
			...
			if (comp >= 0) {
				...
				if (ancestor.balanceFactor == '=') {
					...
					/*
					 * 删除15 调整因子 20 → 20 /L\ /L\ → 10 50 10 50 /=\ /L 5 15 5
					 */
					ancestor.balanceFactor = 'L';
					...
					/*
					 * 删除15 调整因子 下次循环调整20的因子 20 → → 20 ← ancestor → ... /L\ /L\
					 * → 10 50 10 50 /R\ R\ /=\ R\ 5 15 60 5 18 60 R\ 18
					 */
					ancestor.balanceFactor = '=';
					...
					if (ancestor.left.balanceFactor == '=') {
						...
						/*
						 * 删除60 调整因子 绕50右旋 20 → → 20 → 20 /R\ / \ /R\ 10 50 ←
						 * ancestor 10 50 ← 10 45 /L /L\ / /L\ /L /R\ 5 45 60 5
						 * 45 60 5 35 50 ← /=\ /R\ /L 35 48 35 48 48
						 */
						...
						/*
						 * 删除60 调整因子 绕50右旋 下次循环调整20的因子 20 → → 20 → 20 ← p → ...
						 * /R\ / \ /R\ 10 50 ← ancestor 10 50 ← 10 45 /L /L\ /
						 * /=\ /L /=\ 5 45 60 5 45 60 5 35 50 ← ancestor /L /= =
						 * 35 35
						 */
						...
						if (ancestor.left.right.balanceFactor == 'L') {
							...
							/*
							 * 删除60 调整因子 20 → 20 /R\ / \ 10 50 ← ancestor 10 50
							 * ← ancestor /L\ /L\ / \ /R\ 5 12 45 60 5 12 45 70
							 * /L /R\ R\ / /=\ 3 42 48 70 3 42 48 /L /= 46 46
							 * 
							 * 绕45左旋 绕50右旋 下次循环调整20的因子 → 20 → 20 ← p → ... /R\
							 * /R\ 10 50 ← 10 48 /L\ /R\ /L\ /=\ 5 12 48 70 5 12
							 * 45 50 ← ancestor /L /= /L /=\ R\ 3 45 3 42 46 70
							 * /=\ 42 46
							 */
							ancestor.balanceFactor = 'R';
							ancestor.left.balanceFactor = '=';
							...
							/*
							 * 删除60 调整因子 20 → 20 /R\ / \ 10 50 ← ancestor 10 50
							 * ← /L\ /L\ / \ /=\ 5 12 45 60 5 12 45 70 /L /R\ R\
							 * / /L\ 3 42 48 70 3 42 48 R\ =\ 49 49
							 * 
							 * 绕45左旋 绕50右旋 下次循环调整20的因子 → 20 → 20 ← p → ... /R\
							 * /R\ 10 50 ← 10 48 /L\ /=\ /L\ /=\ 5 12 48 70 5 12
							 * 45 50 ← ancestor /L /=\ /L /L /=\ 3 45 49 3 42 49
							 * 70 /L 42
							 */
							ancestor.balanceFactor = '=';
							 ...
							 * 删除60 调整因子 20 → 20 /R\ / \ 10 50 ← ancestor 10 50
							 * ← /L\ /L\ / \ /=\ 5 12 45 60 5 12 45 70 /L /R\ R\
							 * / /=\ 3 42 48 70 3 42 48 /=\ /=\ 46 49 46 49
							 * 
							 * 绕45左旋 绕50右旋 下次循环调整20的因子 → 20 → 20 ← p → ... /R\
							 * /R\ 10 50 ← 10 48 /L\ /=\ /L\ /=\ 5 12 48 70 5 12
							 * 45 50 ← ancestor /L /=\ /L /=\ /=\ 3 45 49 3 42
							 * 46 49 70 /=\ 42 46
							 */
							ancestor.balanceFactor = '=';
							ancestor.left.balanceFactor = '=';

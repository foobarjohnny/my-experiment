...
<jsp:include
		page="/touch62/jsp/ac/controller/EditRouteController.jsp"></jsp:include>
<%
	...
	<![CDATA[
        ...
        func DriveToControl_driveToActionViaJSON(JSONObject jo,String navType)
        	DriveTo_M_setDoingNav(0)
      		...
      		#Fix bug TN 6.2 Sprint Sanyo Orange ORANGE-26 
      		#XinRong add route style info in resume trip node(child 1,2,3) 
      		if JSONObject.has(jo, "isResumeTrip")
	      		TxNode resumeNode = ShareData.get("<%=Constant.StorageKey.SELECT_ADDRESS_RESUMETRIP%>")
	      		if 4 <= TxNode.getChildSize(resumeNode)
	      		   TxNode childNode = TxNode.childAt(resumeNode, 1)
	      		   TxNode.addChild(node, childNode)
	      		   
	      		   childNode = TxNode.childAt(resumeNode, 2)
	      		   TxNode.addChild(node, childNode)
	      		   
	      		   childNode = TxNode.childAt(resumeNode, 3)
	      		   TxNode.addChild(node, childNode)
	      		endif
      		endif

...
<%@page import="com.telenav.cserver.poi.util.PoiListMenuIndex"%>
<%@page
...
<%
	...
	boolean isShareAddressEnabled = featureMgr.isEnabled(FeatureConstant.SHARE_ADDRESS);
	...
	//Update menu index
	PoiListMenuIndex plmi= new PoiListMenuIndex();
	plmi.setFeedBackEnable(isFeedbackEnabled);
	plmi.setShareAddressEnable(isShareAddressEnabled);
	plmi.refreshMenuIndex();
...
<tml:script language="fscript" version="1">
	...
	<%@ include file="../GetServerDriven.jsp"%>
	        ...
	        func setMenuVisible(String itemId,String categoryId,int sortType)
		     		MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.POPULAR%>, 1)
		     		MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.PRICE%>, 1)
		     		MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.RELEVANCE%>, 1)
		     		MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.RATING%>, 1)
		     		MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.DISTANCE%>, 1)
		            if <%=Constant.SORT_BY_RATING%> == sortType
					    MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.RATING%>, 0)
					elsif <%=Constant.SORT_BY_POPULAE%> == sortType
					    MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.POPULAR%>, 0)
					elsif <%=Constant.SORT_BY_RELEVANCE%> == sortType
					    MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.RELEVANCE%>, 0)
					elsif <%=Constant.SORT_BY_GASPRICE%> == sortType
					    MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.PRICE%>, 0)
					else
					    MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.DISTANCE%>, 0)
					endif
	        ...
	        func handleMenuForRate(String itemId, int showRateMenuItem)
				...
				MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.RATE_CONTEXT%>, showRateMenuItem)
				MenuItem.commitSetItemValid(itemId)
			endfunc
	        ...
	        func handleMenuWithLatAndLon(String itemId, int latAndLon)
			    ...
			    if Account.isTnMaps()
			       MenuItem.setItemText(itemId, <%=PoiListMenuIndex.DRIVE_TO%>, "<%=msg.get("poi.get.directions")%>")
			       MenuItem.setItemText(itemId, <%=PoiListMenuIndex.DRIVE_TO_CONTEXT%>, "<%=msg.get("poi.get.directions")%>")
			    else
			       MenuItem.setItemText(itemId, <%=PoiListMenuIndex.DRIVE_TO%>, "<%=msg.get("poi.drive.to")%>")
			       MenuItem.setItemText(itemId, <%=PoiListMenuIndex.DRIVE_TO_CONTEXT%>, "<%=msg.get("poi.drive.to")%>")
			    endif
			    
			    MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.DRIVE_TO%>, latAndLon)
			    MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.MAP%>, latAndLon)
			    MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.SHARE%>, latAndLon)
			    MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.SAVE%>, latAndLon)

				# CONTEXT_MENU FOR ANDROID 6.1
				MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.DRIVE_TO_CONTEXT%>, latAndLon)
			    MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.MAP_CONTEXT%>, latAndLon)
			    MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.SHARE_CONTEXT%>, latAndLon)
			    MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.SAVE_CONTEXT%>, latAndLon)
	    ...
	    <%}%>
	        ...
	        func switchClickMenu(String itemId)
				...
				if 100 == searchType
				   	MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.SELECT%>, 1)
			    	MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.VIEW_DETAILS%>, 0)
				else
					MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.SELECT%>, 0)
			    	MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.VIEW_DETAILS%>, 1)
				endif
				MenuItem.commitSetItemValid(itemId)
	        endfunc
	        ...
	        func handleMenuForPhone(string itemId,string phone)
			   ...
			   if isEmptyString(phone)
			   		  MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.CALL%>, 0)
			   		   # CONTEXT_MENU FOR ANDROID 6.1
			   		  MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.CALL_CONTEXT%>, 0)
			   else
			   	      phone = UTIL_formatPhoneNumber(phone)
					  phone = "<%=msg.get("poi.call")%>" + " " + phone
				      MenuItem.setItemText(itemId, <%=PoiListMenuIndex.CALL%>, phone)
			   		  MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.CALL%>, 1)
			   		  println("~~~~~~~~~~~~~~~phone"+phone)
			   		  # CONTEXT_MENU FOR ANDROID 6.1
		    		  MenuItem.setItemText(itemId, <%=PoiListMenuIndex.CALL_CONTEXT%>, phone)
			   		  MenuItem.setItemValid(itemId, <%=PoiListMenuIndex.CALL_CONTEXT%>, 1)
			   endif
			   MenuItem.commitSetItemValid(itemId)		        
	        endfunc
	        ...
	        func mapResultOnclick()
				...
				<%if(!TnUtil.isATTRIM63(handlerGloble) && !TnUtil.isSprintRim62(handlerGloble)&&!TnUtil.isVNRIM62(handlerGloble)&&!TnUtil.isTMORIM62(handlerGloble)){ %>

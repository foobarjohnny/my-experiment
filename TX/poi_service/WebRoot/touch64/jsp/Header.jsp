...
<%
	...
	String country = PoiUtil.getCountry(handlerGloble);
    String region = handlerGloble.getClientInfo(DataHandler.KEY_REGION);
	List<String> countryList = CommonUtil.getI18NCountryList(region);
	boolean isSupportCountrySelect = TnUtil.isSupportCountrySelect(handlerGloble);
    String imageKeyCommon = (String)request.getAttribute(BrowserFrameworkConstants.IMAGE_KEY_WITHOUT_SIZE) + ClientHelper.getCommonImageFloder(handlerGloble);
    if("".equals(locale))
    {
    ...
    MessageWrap msgFromServer = MessageHelper.getInstance(true).getMessageWrap(msgKey);
	
    // feature manager 
    String[] defaultParams = {DataHandler.KEY_REGION, DataHandler.KEY_CARRIER, DataHandler.KEY_PLATFORM, DataHandler.KEY_VERSION, DataHandler.KEY_PRODUCTTYPE};
    ...
    String getPageWithLocale1 = host + "/goToJsp.do?pageLocale=";
    String getPageWithLocale2 = "&pageRegion=" + region + "&jsp=";
	...
	String purchasePageUrl = "{login.http}/" + ClientHelper.getModuleNameForLogin(handlerGloble) + TnUtil.getLoginFolder(handlerGloble) + "/goToJsp.do?jsp=Purchase";
    ...
    String imageCommonUrl = imageHost + imageKeyCommon + "/image/";

...
import com.telenav.cserver.ac.protocol.ShareAddressRequestParser;
import com.telenav.cserver.ac.protocol.ShareAddressResponseFormatter;
...
public class ShareAddressActionTest {

	private int	ajaxChildValue = 110;
	private String actionName = "shareAddress.do";
	private String failString = "couldn't find the TxNode in file when testing ShareAddress action";
	
	private ActionMapping mapping = new ActionMapping();
	private MockHttpServletRequest request = null;
	private MockHttpServletResponse response = new MockHttpServletResponse();
	private ShareAddressRequestParser parser = new ShareAddressRequestParser();
	private ShareAddressResponseFormatter formatter = new ShareAddressResponseFormatter();
	
	public 	ShareAddressAction shareAddressAction = new ShareAddressAction();
	
	@Before
	public void setUp() throws Exception {
		
		shareAddressAction.setRequestParser(parser);
		shareAddressAction.setResponseFormatter(formatter);
	...
	public void testDoActionAction() {
		try 
		{
			...
			shareAddressAction.doAction(mapping, request, response);

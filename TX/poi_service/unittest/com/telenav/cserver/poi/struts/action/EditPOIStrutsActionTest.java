...
import com.telenav.cserver.poi.protocol.EditPOIRequestParser;
import com.telenav.cserver.poi.protocol.EditPOIResponseFormatter;
...
public class EditPOIStrutsActionTest {
	
	private int	ajaxChildValue = 110;
	private String actionName = "editPOI.do";
	private String failString = "couldn't find the TxNode in file when testing editPOI action";
	
	private ActionMapping mapping = new ActionMapping();
	private MockHttpServletRequest request = null;
	private MockHttpServletResponse response = new MockHttpServletResponse();;	
	private EditPOIRequestParser<?> parser = new EditPOIRequestParser();
	private EditPOIResponseFormatter formatter = new EditPOIResponseFormatter();
	public 	EditPOIStrutsAction editPOIStrutsAction = new EditPOIStrutsAction();


	@Before
	public void setUp() throws Exception {
		
		editPOIStrutsAction.setRequestParser(parser);
		editPOIStrutsAction.setResponseFormatter(formatter);
	...
	public void testDoActionAction() {
		try 
		{
			...
			editPOIStrutsAction.doAction(mapping, request, response);

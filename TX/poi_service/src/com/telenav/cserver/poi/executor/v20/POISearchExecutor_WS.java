 ...
 *  All Rights Reserved.
 */
package com.telenav.cserver.poi.executor.v20;
...
//        }
		...
		//get the instance of configuration file for poi local parameters
        ...
        //add new configuration file, get poi find version from the file first
		String poiFindVersion = CATEGORY_VERSION;
		if(null != plpc && null != plpc.getPoiFindVersion())
		{
			poiFindVersion = plpc.getPoiFindVersion();
		}
        poiReq.setCategoryVersion(poiFindVersion);
		...
		try {
			PoiSearchProxyV20 poiSearchProxy = CoseFactory
			...
			case SEARCH_ALONGROUTE:
				...
				if (searchAlongType == 0) {
					List<GeoCode> routePoints = NSPoiUtil.getRoutePoints(tc,
							...
							poiRequest.getEdgeId(),
							poiRequest.getShapePointId(),
							poiRequest.getRange(), poiRequest.getCurrentLat(),
							poiRequest.getCurrentLon());
					poiReq.setRoutePoints(routePoints);
				} else {
					NSPoiUtil.setReverseRoutePointsAndAnchor(
							context.getTnContext(), poiReq,
							poiRequest.getRouteID(), poiRequest.getRange());
				}
				result = poiSearchProxy.searchAlongRoute(poiReq);
				break;
			case SEARCH_ADDRESS:

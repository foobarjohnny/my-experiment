...
"""test"""
def test():
    doc = urllib.urlopen("http://conceptart.org/forums/showthread.php?83464-Wanimal-s-NSFW/page1")
...
#    print "postList=",postList;
    ...
    for post in posts:
        ...
        if postClass == None: 
            continue
        if postClass.find("postcontainer") != -1:
            print "postClass",postClass
            aa = post.findAll("a")
#            print "aa",aa
            for a in aa:
                if  a.has_key('class'):
                    ...
                    if aClass == None: 
                        continue
                    if aClass.find("username") != -1:
                        print "a",a
                        print "aClass",aClass
                        name = a.find("strong").string
                        if name == "wanimal":
                            
#        post.find(class_=re.compile("[^\"]*postcontainer[^\"]*"))
#    
#    blockquote
    return
#	map = {'1'   : 666,

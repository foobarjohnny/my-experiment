...
my $backup=0;
my $zip_folder="/home/tnuser/zipFolder";
my $zip_name="B-ServerResourceFolders";
my $zip_url="http://tabmaster3.telenav.com:8080/hudson/view/C-Server6x-FB-72/job/B-Server7x-BuildAll-ResourceFolders-FB-72-bserverOnly/lastSuccessfulBuild/artifact/";
processArgs();
...
my $ZIP_URL_KEY='ZIP_URL';
my $ZIP_FOLDER_KEY='ZIP_FOLDER';
my $ZIP_NAME_KEY='ZIP_NAME';
...
while (<CONFIG>) {
	...
	if($bserver eq $ZIP_URL_KEY){
		$zip_url = $value;
		next;
	}
	
	if($bserver eq $ZIP_FOLDER_KEY){
		$zip_folder = $value;
		next;
	}
	
	if($bserver eq $ZIP_NAME_KEY){
		$zip_name = $value;
		next;
	}
...
printTips("download b-resource zip files from $zip_url  :$zip_name");
downloadzip($zip_name);

printTips("update b-resource zip files to $zip_folder :$zip_name");
updatezip($zip_name);
...
sub downloadzip{ 
	my $zip = shift;
	die "\$zip is a empty string\n" if $zip =~ /^\s*$/;
	print "downloading $zip.zip ...\n ";
    my $url = "$zip_url$zip.zip";
	print "$url\n";
	my $ua = LWP::UserAgent->new;
	my $response = $ua->get($url,":content_file" => $zip.".zip");
	unless($response->is_success){
		die "\nFailed when downloading $zip.zip\nzip file URL: $url\nStatus:".$response->status_line."\n";
	}
	die "fail!" unless -e $zip.".zip";
	print "success!\n";
}
...
sub updatezip{ 
	my $zip = shift;
	die "\$zip is a empty string\n" if $zip =~ /^\s*$/;
	my $dest_zip = "$zip_folder$zip.zip";
	#backup zip
	if($backup){
		rename $dest_zip, $dest_zip.".".&getDateTimeStr();
	}
	if( -e $dest_zip ){
		unlink $dest_zip or die "Could not delete file $dest_zip: $!\n";
		print "delete $dest_zip success.\n";
	}

	rename "$zip.zip", "$dest_zip";
	print "move file $zip.zip to $dest_zip\n\n";
	
	my $ok = `unzip -o  $dest_zip -d $zip_folder`;
	print "unzip $dest_zip\n\n";
	
	$ok = `unzip -o  $zip_folder"commuteResourceFolder.zip" -d $zip_folder`;
	print "unzip ${zip_folder}commuteResourceFolder.zip\n\n";
	
	$ok = `unzip -o  $zip_folder"movieResourceFolder.zip" -d $zip_folder`;
	print "unzip ${zip_folder}movieResourceFolder.zip\n\n";
	
	$ok = `unzip -o  $zip_folder"poiResourceFolder.zip" -d $zip_folder`;
	print "unzip ${zip_folder}poiResourceFolder.zip\n\n";
	
	$ok = `unzip -o  $zip_folder"loginResourceFolder.zip" -d $zip_folder`;
	print "unzip ${zip_folder}loginResourceFolder.zip\n\n";
}

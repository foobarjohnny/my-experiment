...
// 边覆盖点覆盖 里面讲混了 看《离散数学基础之图论.pdf》吧
/**
 * 匹配==边独立集
 */
// http://blog.csdn.net/leolin_/article/details/7199688
// http://www.cppblog.com/jericho/archive/2011/03/05/141175.aspx
// http://blog.csdn.net/a402630999/article/details/7224407
// [网络流]最小路径覆盖问题 (二分图最大匹配, 最大流)
// 题意：
// 有向无环图中，需要多少条简单路可以覆盖整个图。
//
// 建模：
// 构造二分图。把原图的每个顶点i拆分成二分图X，Y集合中的两个顶点Xi和Yi。对于原图的边(i, i)，连接(Xi,
// Yj)，值为1，然后把二分图最大匹配模型转化为网络留模型，求最大流。
//
// 分析：
//
// 对于一个路径覆盖，有如下性质：
...
// 如果匹配数为0，那么显然路径数=顶点数。每增加一条匹配边，那么路径覆盖数就减少一个，所以路径数=顶点数
// - 匹配数。要想使路径数最少，则应最大化匹配数，所以要求二分图的最大匹配。
//
// 注意，此建模方法求最小路径覆盖仅适用于有向无环图，如果有环或是无向图，那么有可能求出的一些环覆盖，而不是路径覆盖。
//
// 最小边覆盖 = 最大独立集 = |原图V| - 最大匹配数
// 这个是在原图是二分图上进行的
// 最小路径覆盖和最小边覆盖不同，不要求给的图是二分图，而是要求是N x N的有向图，不能有环，
// 然后根据原图构造二分图，构造方法是将点一分为二，如，i分为i1和i2然后如果i和j有边，那么就在i1和j2之间连一条边。由此构成二分图
...
// = |V| - 最大匹配数
// 【最小边覆盖(原图是二分图)】：在图中找一些边，使之覆盖了图中所有顶点，且任何一个顶点有且只有一条边与之关联。最大点独立集= 最小边覆盖? = |V| -
// 最大匹配数

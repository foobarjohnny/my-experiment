...
public class ExpiresFilter implements Filter
{
     ...
     * {@link #cacheControlHeader} values access to the slow {@link #getHeader(String)} and to spare the <tt>string</tt>
    ...
    public class XHttpServletResponse extends HttpServletResponseWrapper
    {
        ...
        /**
         * Value of the <tt>Last-Modified</tt> http response header if it has been set.
         */
        ...
        /**
         * Indicates whether calls to write methods (<tt>write(...)</tt>, <tt>print(...)</tt>, etc) of the response body
     ...
     * 
     * @param commaDelimitedInts can be <code>null</code>
    ...
    /**
     * Return <code>true</code> if the given <code>str</code> contains the given <code>searchStr</code>.
     */
    ...
    /**
     * Return <code>true</code> if the given <code>str</code> is <code>null</code> or has a zero characters length.
     */
    ...
    /**
     * Return <code>true</code> if the given <code>str</code> has at least one character (can be a withespace).
     */
    ...
    /**
     * Return <code>true</code> if the given <code>string</code> starts with the given <code>prefix</code> ignoring
     * case.
     * 
     * @param string can be <code>null</code>
     * @param prefix can be <code>null</code>
     */
    ...
    /**
     * Return the subset of the given <code>str</code> that is before the first occurence of the given
     * <code>separator</code>. Return <code>null</code> if the given <code>str</code> or the given
     * <code>separator</code> is null. Return and empty string if the <code>separator</code> is empty.
     * 
     * @param str can be <code>null</code>
     * @param separator can be <code>null</code>
     */
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException
    {
        if (request instanceof HttpServletRequest && response instanceof HttpServletResponse)
        {
            ...
            if (response.isCommitted())
            {
                ...
                {
                    log.debug("responseAlreadyCommited: " + httpRequest.getRequestURL());
                }
    ...
    /**
     * <p>
     * Returns the expiration date of the given {@link XHttpServletResponse} or <code>null</code> if no expiration date
     * has been configured for the declared content type.
     * </p>
     * <p>
    ...
    protected Date getExpirationDate(XHttpServletResponse response)
    {
        ...
        if (configuration != null)
        {
            ...
            if (log.isDebugEnabled())
            {
                // log.debug("useMatchingConfiguration", configuration, contentType, contentType, result));
            }
            return result;
        }
        ...
        if (contains(contentType, ";"))
        {
            ...
            if (configuration != null)
            {
                ...
                if (log.isDebugEnabled())
                {
                    ...
                    // contentTypeWithoutCharset, contentType,
                    // result));
                }
                return result;
            }
    ...
    protected Date getExpirationDate(ExpiresConfiguration configuration, XHttpServletResponse response)
    {
        ...
        switch (configuration.getStartingPoint())
        {
            ...
            case LAST_MODIFICATION_TIME:
                ...
                break;
            default:
                throw new IllegalStateException("unsupportedStartingPoint: "+configuration.getStartingPoint());
        }
    ...
    public void init(FilterConfig filterConfig) throws ServletException
    {
        for (Enumeration<String> names = filterConfig.getInitParameterNames(); names.hasMoreElements();)
        {
            ...
            {
                throw new ServletException("exceptionProcessingParameter:\n" + name + ":" + value, e);
            }
        }
    ...
    protected boolean isEligibleToExpirationHeaderGeneration(HttpServletRequest request, XHttpServletResponse response)
    {
        ...
        if (expirationHeaderHasBeenSet)
        {
            if (log.isDebugEnabled())
            {
               // log.debug(sm.getString("expiresFilter.expirationHeaderAlreadyDefined", request.getRequestURI(), Integer.valueOf(response
        ...
        /*for (int skippedStatusCode : this.excludedResponseStatusCodes)
        {
            if (response.getStatus() == skippedStatusCode)
            {
                if (log.isDebugEnabled())
                {
                    //log.debug(sm.getString("expiresFilter.skippedStatusCode", request.getRequestURI(), Integer
                    //        .valueOf(response.getStatus()), response.getContentType()));
                }
                return false;
            }
        }*/
     ...
     * {@link ExpiresFilter} configuration, sets the ' <tt>Expires</tt>' header and the attribute '<tt>max-age</tt>' of
     * the ' <tt>Cache-Control</tt>' header.
     * </p>
     * <p>
     * Must be called on the "Start Write Response Body" event.
     * </p>
     * <p>
     * Invocations to <tt>Logger.debug(...)</tt> are guarded by {@link Log#isDebugEnabled()} because
     * {@link HttpServletRequest#getRequestURI()} and {@link HttpServletResponse#getContentType()} costs <tt>String</tt>
    ...
    public void onBeforeWriteResponseBody(HttpServletRequest request, XHttpServletResponse response)
    {
        ...
        if (expirationDate == null)
        {
            if (log.isDebugEnabled())
            {
                //log.debug(sm.getString("expiresFilter.noExpirationConfigured", request.getRequestURI(), Integer.valueOf(response
                //        .getStatus()), response.getContentType()));
            }
        }
        else
        {
            if (log.isDebugEnabled())
            {
                //log.debug(sm.getString("expiresFilter.setExpirationDate", request.getRequestURI(), Integer.valueOf(response.getStatus()),
    ...
    /**
     * Parse configuration lines like ' <tt>access plus 1 month 15 days 2 hours</tt>' or '
    ...
    protected ExpiresConfiguration parseExpiresConfiguration(String inputLine)
    {
        ...
        {
            throw new IllegalStateException("expiresFilter.startingPointNotFound");
        }
        ...
        else
        {
            ...
            throw new IllegalStateException("startingPointInvalid");
        }
        ...
        catch (NoSuchElementException e)
        {
            ...
            throw new IllegalStateException("Duration not found in directive '{}'");
        }
    ...
    public String toString()
    {
        return getClass().getSimpleName() + "[excludedResponseStatusCode=[" + intsToCommaDelimitedString(this.excludedResponseStatusCodes)
                + "], default=" + this.defaultExpiresConfiguration + ", byType=" + this.expiresConfigurationByContentType + "]";
    }

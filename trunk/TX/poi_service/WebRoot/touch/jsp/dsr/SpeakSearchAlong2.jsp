...
<%@ include file="/touch/jsp/StopUtil.jsp"%>
	...
	<tml:page id="SpeakSearchAlong2" url="<%=pageURL%>" type="<%=pageType%>" background="" supportback="false" groupId="<%=GROUP_ID_DSR%>">
		<%
			handler.toXML(out);
		%>
		<tml:label id="text1" fontWeight="bold|system_large" align="center"/>
		<tml:label id="label1" fontWeight="system_median" align="center"/>
		<tml:progressBar id="progressBar"/>
		
		<tml:button id="changeLocationButton" text=""
			fontWeight="system_small" isFocusable="true" 
			imageClick=""
			imageUnclick="">

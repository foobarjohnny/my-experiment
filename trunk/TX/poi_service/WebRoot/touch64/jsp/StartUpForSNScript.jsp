...
%>
	...
	<jsp:include page="/touch64/jsp/FreeTrialSDParamScriptForSprint.jsp" />
	...
	<tml:script language="fscript" version="1">
		<![CDATA[
			...
			func pageCallBack(int type)
				# if we add scriptCallback attribute for TmlPageTag, when keyboard events occurs, the callBackMethod will be called.
				# int SCRIPT_EVENT_TYPE_EXIT = 0;
				# int SCRIPT_EVENT_TYPE_ACTION = 1;
				# int SCRIPT_EVENT_TYPE_BACK = 2 ;
				# int SCRIPT_EVENT_TYPE_ONCHANGE = 3;
				# int SCRIPT_EVENT_TYPE_QUIT = 4;
				# int SCRIPT_EVENT_TYPE_ONFOCUS = 5;
				# int SCRIPT_EVENT_TYPE_SWIPE = 6;
				# int SCRIPT_EVENT_TYPE_NATIVE_BROWSER_FINISH_PAGE_LOAD = 7;
				# int SCRIPT_EVENT_TYPE_SYSTEM_CALL_BACK = 8;
				if 3 == type 
					checkMyFavorite()
				elsif 5 == type
					if getFocusButton() == -1
			...
			func preLoad()
				...
				if AddressCapture_M_getCountryListCache_Flag() > 0
					TxNode countryListNode
					<%
						String countryListElem;
						Iterator it = countryList.iterator();
						while( it.hasNext())
						{
							countryListElem = (String)it.next();
					%>
						TxNode.addMsg(countryListNode, "<%=countryListElem%>")
					<%				
						}
					%>	
					AddressCapture_M_saveCountryList(countryListNode)
					AddressCapture_M_saveCountry("<%=country%>")
					AddressCapture_M_setCountryListCache_Flag(0)
				endif
			endfunc
			...
			func onLoad()
			    ...
			    showPopupWhenLoad()
			    <%if(!TnUtil.isRIM64(handlerGloble)){%>
	        ...
	        func checkActionForLastItem()
				...
				# remove the following code until we support adjuggler in 6.4
				# otherwise it will cause exception when executing it
	        ...
	        func checkAppNew()
				...
				# hard code make sure the "new" not shown
				int isNew = 0
	        	if isNew

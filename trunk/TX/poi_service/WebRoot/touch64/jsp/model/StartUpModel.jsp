...
<tml:script language="fscript" version="1">
	<![CDATA[
	    ...
	    func StartUp_M_saveClickUpgrade(String log)
	        TxNode node
	        TxNode.addMsg(node, log)
	        Cache.saveToTempCache("CLICK_MENU_PURCHASE", node)
	    endfunc
	    
	    func StartUp_M_saveAdJuggler(JSONObject adJuggler)
	        String key = StartUp_M_getAdJugglerKy()
			Cache.saveCookie(key, adJuggler)
        endfunc
	    func StartUp_M_getAdJuggler()
	    	String key = StartUp_M_getAdJugglerKy()
	    	JSONObject result = Cache.getJSONObjectFromCookie(key)
	    	return result
	    endfunc
	    
	    func StartUp_M_clearAdJuggler()
	    	String key = StartUp_M_getAdJugglerKy()
	    	Cache.deleteCookie(key)
	    endfunc
	    
	    func StartUp_M_getAdJugglerKy()
	        String key = "ADJUGGLER_JSON_KEY_PREM"
	        if !isPremiumAccount()
	           key = "ADJUGGLER_JSON_KEY_NON_PREM"
	        endif

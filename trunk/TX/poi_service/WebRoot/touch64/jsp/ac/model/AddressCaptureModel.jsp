...
<tml:script language="fscript" version="1">
	<![CDATA[
			...
			func AddressCapture_M_getCountry()
		        String saveKey = "<%=Constant.StorageKey.ADDRESS_CAPTURE_COUNTRY%>"
				return AddressCapture_M_getString(saveKey)
		    endfunc
			...
			func AddressCapture_M_getCountryListCache_Flag()
				String key = "CountryList_Cache_Flag"
				String flag = Cache.getFromTempCache(key)
				if NULL != flag
					return 0
				endif
				return 1
			...
			func AddressCapture_M_setCountryListCache_Flag(int flag)
				String key = "CountryList_Cache_Flag"
				if NULL != flag
					Cache.saveToTempCache(key,flag)
				endif
			endfunc
			
			func AddressCapture_M_getCountryList()
				String saveKey = "<%=Constant.StorageKey.ADDRESS_CAPTURE_COUNTRYLIST%>"
				return Cache.getFromTempCache(saveKey)
			endfunc
			
			func AddressCapture_M_saveCountryList(TxNode countryListNode)
				if NULL != countryListNode
					Cache.saveToTempCache("<%=Constant.StorageKey.ADDRESS_CAPTURE_COUNTRYLIST%>",countryListNode)
				endif
			endfunc

			func AddressCapture_M_saveCountry_ForAcTemplate(TxNode countryNode)
				if NULL != countryNode
					Cache.saveCookie("<%=Constant.StorageKey.ADDRESS_CAPTURE_COUNTRY_FOR_REFRESH_AC_TEMPLATE%>",countryNode)
				endif
			endfunc
			
			func AddressCapture_M_getCountry_ForAcTemplate()
				String country = ""
				TxNode countryNode
		        countryNode = Cache.getCookie("<%=Constant.StorageKey.ADDRESS_CAPTURE_COUNTRY_FOR_REFRESH_AC_TEMPLATE%>")
				if NULL != countryNode 
					if TxNode.getStringSize(countryNode) > 1
						country = TxNode.msgAt(countryNode, 1)
					endif
				else
					country = AddressCapture_M_getCountry()
			    endif
				return country
		    endfunc
...
#			endfunc
		    	   ...
		    	   if "1" == isCityChange || NULL == nearCityMsgNode
					  ...
					  if NULL != nearCityNode
					     ...
					     while i < childSize
					        ...
					        cityStr = TxNode.msgAt(childNode,2)
					        statesStr = TxNode.msgAt(childNode,3)
						    localityStr = TxNode.msgAt(childNode,8)
						    districtStr = TxNode.msgAt(childNode,9)
							if NULL != localityStr && "" != localityStr
							    cityMsg = localityStr + ", "
							endif
							if NULL != cityStr && "" != cityStr
								cityMsg = cityMsg + cityStr + ", "
							endif
							if NULL != districtStr && "" != districtStr
								cityMsg = cityMsg + districtStr+", "
							endif
					        if NULL != cityMsg && "" != cityMsg
					            if NULL != statesStr && "" != statesStr
					                cityMsg = cityMsg + statesStr
								else
								    int length = String.getLength(cityMsg)
								    cityMsg = cityMsg.subString(0, length-2)
					            endif
			...
			func EditHome_M_FromHome()
				TxNode node
				String saveKey = "EditHome_M_FromHome"
				Cache.saveToTempCache(saveKey,node)	
			endfunc

			func EditHome_M_isFromHome()
				String saveKey = "EditHome_M_FromHome"
				if NULL != Cache.getFromTempCache(saveKey)
					return TRUE
				endif
				return FALSE
			endfunc
			
			func EditHome_M_DeleteFromHome()
				String saveKey = "EditHome_M_FromHome"
				if NULL != Cache.getFromTempCache(saveKey)
					Cache.deleteFromTempCache(saveKey)
				endif
			...
			func EditHome_M_saveCountry(String country)
				String saveKey = "EditHome_M_Country"
				TxNode node
				TxNode.addMsg(node,country)
				Cache.saveToTempCache(saveKey,node)
			endfunc
			
			func EditHome_M_getCountry()
				String saveKey = "EditHome_M_Country"
				TxNode node = Cache.getFromTempCache(saveKey)
				if NULL != node

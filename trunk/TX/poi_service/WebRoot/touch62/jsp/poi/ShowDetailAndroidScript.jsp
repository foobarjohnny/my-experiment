	...
	<%@ include file="ShowRateImageScript.jsp"%>
		...
		<%}%>
			...
			func showDetail(int sponsor)
			    if Account.isTnMaps()
			       Page.setComponentAttribute("driveToB","text","<%=msg.get("maps.direction")%>")
			    else
			       Page.setComponentAttribute("driveToB","text","<%=msg.get("poi.drive.to")%>")
			    endif
				   ...
				   if JSONObject.has(locationJo,"ratingNumber")
					   int ratingNumber = JSONObject.get(locationJo,"ratingNumber")
					    if 0 != ratingNumber
					       ...
					       if ratingNumber == 1
						        String onerating = "<%= msg.get("poi.detail.onerating")%>"
				...
				#fix bug ATTBBPORT-109
				println("address="+address+"#")
				if String.trim(address) == " "
					latAndLon = 0
				endif
				...
				if (JSONObject.has(locationJo,"ad") || JSONObject.has(locationJo,"gasPirce")) 
					String mContent = ""
					if (JSONObject.has(locationJo,"gasPirce")) && 1 == ServerDriven_CanGasByPrice()
				...
				#for canada carrier, Ads is not supported
		...
		<%if (featureMgr.isEnabled(FeatureConstant.MOVIE)) {%>
				...
				if "<%=Constant.MOVIE_THEATER_CATEGORY_ID%>" == poiCategoryId
					...
					MenuItem.setItemValid("reservation", 1, TRUE)
					...
					MenuItem.setItemValid("reservation", 1, FALSE)
		...
		<% } %>
				...
				MenuItem.commitSetItemValid("reservation")
			...
			func displayRestaurant(JSONObject locationJo)
				if ServerDriven_CanRestaurant()
					...
					if JSONObject.has(locationJo,"restaurant")
			...
			func hideMenuWhenNoLatLon(int latAndLon,int hidePhoneMenu)
			    ...
			    Page.setComponentAttribute("shadowBg","visible",latAndLon + "")
			    Page.setComponentAttribute("controlBg","visible",latAndLon + "")
			endfunc

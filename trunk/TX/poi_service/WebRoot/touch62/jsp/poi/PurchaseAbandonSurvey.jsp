...
<tml:TML outputMode="TxNode">
	...
	<% if( TnUtil.isSprint62(handlerGloble) ) { %>
		...
		<![CDATA[
			...
			# get survey answer node
				...
				if 0 == validateAnswers(nodeValue)
					System.showErrorMsg("<%=msgTemp.get("generic.feedback.selectFeedback")%>")
					return FAIL
				else		
					TxRequest req
			...
			func feedbackSaveCallback(TxNode node,int status)
				...
				elseif status == 1
					...
					string msg = "<%=msgTemp.get("generic.feedback.submitSuccess")%>"
			...
			func Callback_PopopTimeOut(int param)
				<% if( TnUtil.isSprint62(handlerGloble) ) { %>
					if FALSE == AccountTypeForSprint_IsBundleUser()
					   System.quit()
					endif
					#exit if it is not non-bundle account 
				<% } %>	

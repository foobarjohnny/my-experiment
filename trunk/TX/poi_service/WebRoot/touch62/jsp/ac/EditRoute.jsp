...
<%
	...
	String loginMainPageUrl = "{login.http}/" + ClientHelper.getModuleNameForLogin(handlerGloble) + "/startup.do";
%>
...
<tml:TML outputMode="TxNode">
	...
	<jsp:include
		page="/touch62/jsp/ac/controller/SelectAddressController.jsp" />
	...
	<tml:script language="fscript" version="1">
		<![CDATA[
           ...
           func chooseLocation()
	        	...
	        	<%if(TnUtil.isBAWUser(handlerGloble)){%>
          ...
          func DriveToControl_driveToActionViaJSON(JSONObject jo)
        	...
        	RecentPlace_saveAddress(originStop)
        	JSONObject destStop =JSONObject.get(jo,"dest")
        	RecentPlace_saveAddress(destStop)
      		TxNode originNode 
			...
			if(JSONObject.getInt(stop,"type")==6)
				...
				if firstLine == ""
				    if  city != ""
						...
						firstLine= JSONObject.get(stop,"state")
					endif
				else
	...
	endfunc 
        ...
        func preLoad()
			...
			if "" != avoid_pref
       			Page.setComponentAttribute("routeStyle","text",routeStyle_pref + "; "+ avoid_pref)
			else
				Page.setComponentAttribute("routeStyle","text",routeStyle_pref)
			endif
       		#Page.setComponentAttribute("avoid","text",avoid_pref)
			<%if(TnUtil.isUSCCRIM62(handlerGloble)){%>
			TxNode node
			String phoneNumber=ServerDriven_GetUpgradePhone()
			TxNode.addMsg(node,phoneNumber)
			MenuItem.setBean("upgrade", "phonenumber", node)
			<% } %>
        endfunc
	...
	<tml:menuItem name="home" pageURL="<%=host + "/startUp.do?pageRegion=" + region%>">
	 ...
	 <%if(TnUtil.isUSCCRIM62(handlerGloble)){%>
		...
		<tml:menuItem name="upgrade" actionRef="makePhoneCallAction" text="<%=msg.get("about.support.menu.phonecall")%>" trigger="KEY_MENU|TRACKBALL_CLICK"/>
	 <% } else { %>
	<tml:menuItem name="upgrade" pageURL="<%=loginMainPageUrl%>"
	...
	<tml:page id="EditRoute" url="<%=pageURL%>" type="<%=pageType%>"
		...
		<tml:urlLabel id="upgradeLink" isFocusable="true"
			align="center" fontColor="blue">
			<%=msg.get("maps.upgrade")%>
			<tml:menuRef name="upgrade"></tml:menuRef>
		</tml:urlLabel>
		<tml:multiline id="upgradeLabel" align="center">

...
<tml:TML outputMode="TxNode">
	...
	<![CDATA[
		...
		func preLoad()
		    <%if(TnUtil.isRim(handler)) {%>
		    	Page.setComponentAttribute("typeInButton","focused","true")
		    <%}else{%>
		    	Page.setComponentAttribute("typeInButton","focused","false")
		    <%}%>
		endfunc
	...
	<tml:page id="SpeakCommand" url="<%=getDsrPage + "SpeakCommand1"%>" type="<%=pageType%>" background="" 
		...
		<tml:button id="typeInButton" text="<%=msg.get("dsr.finish")%>"
			fontWeight="system_median" isFocusable="false" visible="1"
			imageClick=""
			imageUnclick="">

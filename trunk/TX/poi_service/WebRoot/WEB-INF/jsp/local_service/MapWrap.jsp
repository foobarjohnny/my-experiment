...
<%
	...
	String commuteAlertUrl = "{commuteAlert.http}/goToJsp.do?pageRegion=" + region + "&amp;jsp=Startup";
...
<%@ include file="/WEB-INF/jsp/ac/controller/AddressCaptureController.jsp"%>
	...
	<%@ include file="/WEB-INF/jsp/local_service/model/MapWrapModel.jsp"%>
	<%@ include file="/WEB-INF/jsp/controller/DriveToController.jsp"%>
			<%@ include
		file="/WEB-INF/jsp/controller/ATTExtrasController.jsp"%>
	...
	<jsp:include page="/WEB-INF/jsp/poi/StatLogger.jsp" />
	<%@ include
		file="/WEB-INF/jsp/ac/controller/ShareAddressController.jsp"%>
	<%@ include file="/WEB-INF/jsp/poi/controller/SearchPoiController.jsp"%>
	<%@ include file="/WEB-INF/jsp/poi/controller/ShowDetailController.jsp"%>
	<%@ include file="/WEB-INF/jsp/poi/controller/PoiListController.jsp"%>
	<%@ include file="/WEB-INF/jsp/ac/controller/RecordLocationController.jsp"%>
	<%@ include file="/WEB-INF/jsp/ac/controller/CreateFavoritesController.jsp"%>
	<%@ include file="/WEB-INF/jsp/model/DriveToModel.jsp"%>
	<jsp:include page="/WEB-INF/jsp/common/commute/controller/CommuteAlertController.jsp" />
...
<%}%>		
		...
		func driveTo(JSONObject status) 
			...
			if NULL != poi
				address = JSONObject.get(poi,"stop")
			else
				address = getAddressFromMapStatus(status)
			endif
			...
			TxNode addressNode
			String addressStr = JSONObject.toString(address)
			TxNode.addMsg(addressNode, addressStr)
			DriveTo_C_doNav(addressNode, navType, "MAP")
		...
		func RGCCallback(TxNode node,int status)
			...
			TxNode addressNode
			TxNode.addMsg(addressNode, addressStr)
			DriveTo_C_doNav(addressNode, "", "MAP")  
		endfunc		
	...
	<tml:script language="fscript" version="1"
		...
		func shareLocation(JSONObject status)
			...
			if NULL != poi
				...
				JSONObject address
				address = getAddressFromMapStatus(status)		
				JSONObject.put(address, "firstLine", "<%=msg.get("map.cursoraddress")%>")
